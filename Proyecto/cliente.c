...
void lista( void ) {
	...
	char *idLista = zstr( (char*)"" );
 	...
 	if ( true == sintaxisLista( &idLista ) ) {
		...
		// recibir cuantos registros manda el servidor
		int total = 0;
		read( clisock, &total, sizeof( int ) );

		if( total == 0 ) {
			mensajeConLinea( "Lista vacia" );
		} else {
			char msj[ 50 ] = { '\0' };
			sprintf( msj, "Lista '%s' - %d registro/s:", zstrupr( idLista ), total );
			mensajeConLinea( msj );

			for( int j = 0; j < total; j++ ) {

				// recibir el string serializado
				size_t len = 0;
				read( clisock, &len, sizeof( size_t ) );
				int pack = calcPack( len );
				...
				// dependiendo del codigo mostrar cliente o mensaje
				if( strcasecmp( idLista, "C" ) == 0 ) {
...
void initSesion( char *ip ) {
	...
	printf( "\033[0m@" );
	printf( "\033[01;34m%s", ip != NULL ? ip : "127.0.0.1" ); 
	printf( "\033[0m> " );
}
...
main( int argc, char* args[] ) {
	...
	if( opts[ 1 ] == NULL ) opts[ 1 ] = (char*)"127.0.0.1";
	initSocket( opts[ 1 ] );
	...
	for(;;) {
    	...
    	if( cmd == NULL || strlen( cmd ) == 0 || !strcasecmp( cmd, "salir" ) ) {
