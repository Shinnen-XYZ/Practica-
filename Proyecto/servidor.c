...
static void cmd_lista( int sock, char *codigo ) {
    if (strcasecmp(codigo, "C") == 0) {
        printf("[Servidor] Lista de clientes solicitada\n");
        pthread_mutex_lock(&mtxClientes);

        // contar cuantos registros hay
        int total = 0;
        Nodo *n = iniClientes;
        while (n) { total++; n = n->ptr_sig; }

        // mandar la cantidad primero
        write(sock, &total, sizeof(int));

        // mandar cada cliente serializado
        n = iniClientes;
        while (n) {
            char *s = serializarCliente((InfoCliente*)n->dato);
            enviarPaquetes(sock, s);
        ...
        printf("[Servidor] Lista de mensajes solicitada\n");
        pthread_mutex_lock(&mtxMensajes);
