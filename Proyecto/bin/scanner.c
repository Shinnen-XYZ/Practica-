...
int parStr( int conComa, char **resultado, const char *sep ) {
    ...
    /* Validar que el token actual (lookahead) sea comilla */
    if ( token != '"' ) {
        mensaje_error("Se esperaba comilla doble de apertura");
        return false;
    }
    /* Consumir la comilla de apertura (el lookahead la tiene pendiente) */
    lee_car(); /* ahora token = '"' real */
    /* Entrar al contenido del string */
    lee_car();
